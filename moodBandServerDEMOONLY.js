...
var init = function () {
  ...
  function makeRequest() {
    ...
    if(demomode) {
       var step_size = 0.02;
       onReqData("{\"attention\": " + constrain(stateOfMind.attention + (Math.random() <= 0.5 ? -step_size : step_size ), 0.0, 0.75) + "," + 
                     "\"meditation\": " + constrain(stateOfMind.meditation + (Math.random() <= 0.5 ? -step_size : step_size ), 0.0, 0.75) + "," + 
                     "\"bored\": " + constrain(stateOfMind.bored + (Math.random() <= 0.5 ? -step_size : step_size ), 0.0, 0.75) + "," +
                     "\"frust\": " + constrain(stateOfMind.frust + (Math.random() <= 0.5 ? -step_size : step_size ), 0.0, 0.75) + "," +
                     "\"med\": " + constrain(stateOfMind.med + (Math.random() <= 0.5 ? -step_size : step_size), 0.0, 0.75) + "," +
                     "\"excite\": " + constrain(stateOfMind.excite + (Math.random() <= 0.5 ? -step_size : step_size ), 0.0, 0.75) + "}")
    }
  }
